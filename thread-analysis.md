...
### 7.2 A desk-rejection threshold — on quality, not provenance

Journals have **desk rejection**: the editor rejects a manuscript without full peer review when quality is evidently below the bar. Kernel review has no formalized equivalent — the closest things in this thread were netdev CI and Jakub's RFC-downgrade, both far short of a rejection gate. The README of this case study already states the instinct: "an average maintainer would normally have rejected this patchset after the first 5 to 10 bugs." The proposal: **below a threshold of evident quality, the series is rejected wholesale, with the burden of proof shifted back to the submitter** (composable with the verified-submission gates of Rajat's talk in this same BoF).

One tempting wrong turn to head off explicitly. Since this patchset never declared itself AI-generated or AI-assisted (§5), the reflex is to reach for tools that *detect* AI-generated code and gate on that. Wrong target: **code should not be rejected because it is AI-generated; it should be rejected because it is badly written.** A provenance gate is unreliable (detector arms race), unfair in both directions (low-quality human code passes, high-quality AI code is punished), and unnecessary — because on this series, provenance-neutral, *mechanically checkable* quality signals were abundant, each sufficient to stop the review at the desk:

- **Duplicated code.** Five near-identical implementations of the same GTP-U parse/strip/push logic — "nearly identical each time" (Andrea, cover letter). Token-based clone detection is decades-old technology; a duplication density this high inside a single series is a desk-reject on its own.
- **Variable and cast patterns no human writes** — and none maintains happily: explicit casts on `skb_push()`, which returns `void *`; `(unsigned int)` casts on int prefix lengths already range-checked at parse time; `(void)teid; (void)qfi;` dead-code casts; an intermediary variable (`gtp`) existing only to be cast; dead initializers (`reason`); defensive checks that cannot trigger and would fail silently if they did. Each one a nit; their *density* is the fingerprint of unreviewed generation — and standard static analysis (dead stores, useless casts, unreachable branches) counts it mechanically.
- **Commit-message/diff consistency.** The message lists a drop reason the diff never adds — literally a grep (it was Sashiko's cheapest true positive). Same class: a *required* attribute unused in the patchset's own documented example (`src` with `v4_mask_len 32` — no author who ran their own example can produce this); comments describing absent code (the hop-limit decrement, the LOCAL_OUT claim).
- **Scale and structure.** Six behaviors in one series, 5,155 lines, functions well past 100 lines, style drift measurable against the surrounding file.

That these same signals also scream "low-effort AI generation with poor prompting" is *diagnostic, not normative*: the code is rejected for what it is, not for who or what wrote it. This framing keeps disclosure an orthogonal honesty norm, sidesteps the AI-detection arms race entirely, and — decisively for the BoF — is implementable **today** as CI/Patchwork automation, with maintainers owning the thresholds the same way they own coding style. None of these gates requires finding a single bug: their job is to *price the review before it starts*. A desk-rejection threshold is not anti-AI; it is review-economics. It protects the scarce resource (§6.6) that the cost asymmetry attacks: expert maintainer attention.
...
9. **Proposals (§7) + discussion hook** — two constructive moves: (a) improve AI review where the margins evidently are: feedback loops on debunked findings ("% wrong" next to "% unique") and maintainer-owned per-subsystem review prompts (the mechanism already exists — review-prompts — the pipeline doesn't); (b) **a desk-rejection threshold on quality, not provenance** — the journal analogy, with the explicit twist: don't reject code because it's AI-generated, reject it because it's badly written (no AI-detector arms race needed). This series carried mechanical, provenance-neutral desk-reject signals: clone density (×5 near-identical logic), cast/dead-code patterns no human writes, commit-message/diff mismatches, the required-but-unused attribute in its own documented example. Reject wholesale below the bar and return the cost to the sender. Hook for the room: who defines the bar, and where does it live (CI? Patchwork? MAINTAINERS)?
